...
class SynthesisNetwork(torch.nn.Module):
    ...
    def forward(self, ws, f_latents=None, f_layer=0, mode="wp", **layer_kwargs):
        if mode == "wp":
            return self.forward_wp(ws, **layer_kwargs)
        elif mode == "from_f":
            return self.forward_f(ws, f_latents, f_layer, **layer_kwargs)
        elif mode == "to_f":
            return self.forward_from_f(ws, f_layer, **layer_kwargs)
    ...
    def forward_from_f(self, ws, f_latents, f_layer, **layer_kwargs):
        ...
        x = f_latents
        for name, w in zip(self.layer_names[f_layer:], ws[1+f_layer:]):
            x = getattr(self, name)(x, w, **layer_kwargs)
    ...
    def forward_to_f(self, ws, f_layer, **layer_kwargs):
        ...
        # Execute layers.
        x = self.input(ws[0])
        for name, w in zip(self.layer_names[:f_layer], ws[1:1+f_layer]):
            x = getattr(self, name)(x, w, **layer_kwargs)
